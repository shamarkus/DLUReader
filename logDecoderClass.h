#ifndef LOGDECODERCLASS_H_
#define LOGDECODERCLASS_H_
...
static int ATPNullishArray[ATP_BYTE_NUM_TO_SKIP][2] = {{1},{85,87},{80},{-1}};
...
class fileParsingInfo {
	...
	void parseFile();
	void parseLine(char* curHeader, char* curLine, char (*curParams)[MAX_SHORT_STRING_SIZE]);
	void printHeader(int numParameters);
	void printLine(char (*curParams)[MAX_SHORT_STRING_SIZE],int numParameters);
};
